...
# Default Fibonacci retracement ratios for confluence (tightened: drop 0.382).
# Override via Config.fib_levels / env FIB_LEVELS.
FIB_RATIOS: Tuple[float, ...] = (0.5, 0.618)
...
class FibSwing:
    ...
    Upswing (bullish structure): swing_low -> swing_high (low index < high index).
      ...
      LONG wants price near one of these supports (pullback into 0.5-0.618).
    ...
    Downswing (bearish structure): swing_high -> swing_low (high index < low index).
      ...
      SHORT wants price near one of these resistances (bounce into 0.5-0.618).
    """
...
def fib_confluence(
    ...
    LONG: require an upswing in the lookback and price near a configured Fib
          support of that up move (default 0.5 / 0.618 pullback).
    SHORT: require a downswing and price near a configured Fib resistance of
           that down move (default 0.5 / 0.618 bounce).
