...
# Classic Fibonacci retracement ratios used for confluence.
...
class FibSwing:
    ...
    def retracement_levels(self) -> List[Tuple[float, float]]:
        """Return [(ratio, price), ...] for 0.382 / 0.5 / 0.618 of this swing."""
        rng = self.range
        if rng <= 0:
            return []
...
def fib_confluence(
    ...
    LONG: require an upswing in the lookback and price near a 0.382/0.5/0.618
          support of that up move (pullback).
    SHORT: require a downswing and price near a 0.382/0.5/0.618 resistance of
           that down move (bounce).
    ...
    levels = swing.retracement_levels()
    if not levels:
